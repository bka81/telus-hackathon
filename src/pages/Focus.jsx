...
export default function Focus() {
  ...
  const apiTasks = location.state?.tasks || [];
  ...
  const displayTasks = apiTasks.length > 0 ? apiTasks : [
    ...
    { text: "Set an alarm", detail: "Give yourself a 15-minute window to rest before the next step." }
  ];

  const currentTask = displayTasks[currentIndex];

  // TRIGGER: Wiggle whenever the step changes
  useEffect(() => {
    setIsWiggling(true);
    const timer = setTimeout(() => setIsWiggling(false), 600); // Wiggle for 600ms
    return () => clearTimeout(timer);
  }, [currentIndex]); // Dependency array ensures this runs on every new step
      ...
      <div className="w-full flex flex-col items-center justify-start pt-20 px-6 z-10 min-h-screen">
        ...
        <div className="w-full max-w-md bg-white rounded-[2.5rem] p-10 shadow-xl text-center space-y-8 animate-in zoom-in duration-300">
          ...
          <div className="space-y-4">
            ...
            >
              {/* THE WAND: Bounces/Wiggles on arrival, tilts on hover */}
              <span className={`text-2xl transition-all duration-300 inline-block
